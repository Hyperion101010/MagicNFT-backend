...
export class UserController {
    ...
    @Post('creatorrequest')
    async creatorrequestchange(@Req() req: Request) {
        ...
        return await this.userService.resetUserData(resetUserDto);
        // console.log(resetUserDto);
    }

    @Post('find-user')
    async finduserbyaccount(@Req() req: Request) {
        const findData = { account: req.body.accountAddress };
        return await this.userService.findByAccount(findData);
    }

    @Post('creatormanage')
    async creatoraccepted(@Req() req: Request) {
        let tempInviteCode;
        if( req.body.accepted == 'OK' )
        {
            tempInviteCode = uuid();
            const resetCreatorDataDto = { email: req.body.email, inviteCode: tempInviteCode };
            return await this.userService.resetCreatorData(resetCreatorDataDto);
        }
        return false;
    }

    @Post('url-verify')
    async verifyUrl(@Req() req: Request) {
        // console.log(req.body.verifyToken);
        return await this.userService.verifyEmailFromDatabase(req.body.verifyToken);
    }
