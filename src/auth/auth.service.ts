...
export class AuthService {
  ...
  async createJwtAccessToken(email: string, inviteCode: string) {
    const jwtToken = sign({ email: email, inviteCode: inviteCode }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRATION });
    return this.encryptText(jwtToken);
  }

  async hashCreatorData(token: string) {
  ...
  private exampleJwtExtractor(request) {
    try {
      let token = request;
      const cryptr = new Cryptr(process.env.ENCRYPT_JWT_SECRET);
      token = cryptr.decrypt(token);
